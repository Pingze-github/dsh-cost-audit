...
if [ "$GUI" = "1" ]; then
  ...
  # One script, both views, its own retry and its own verdict — see gui-views.mjs
  # for why an unverifiable run is a note rather than a pass.
  node scripts/gui-views.mjs --url "$TOKEN_URL" --session "$SESSION"
fi
